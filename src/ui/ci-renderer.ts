...
export class CIRenderer implements Renderer {
  ...
  onRoundUpdate(state: DisplayState): void {
    // Only log on round completion (status === 'done' or 'failed')
    for (const [, rd] of state.rounds) {
      if (rd.status === 'done') {
        const tokenStr = rd.tokens !== undefined ? formatTokens(rd.tokens) : '';
        const costStr = rd.cost !== undefined ? formatCost(rd.cost) : '';
  ...
  onComplete(state: DisplayState): void {
    ...
    console.log(
      `${this.timestamp()} [done] ${state.completionDocs} documents, ${formatTokens(state.totalTokens)}, ${formatCost(state.totalCost)}, ${duration}`,
    );
