...
export function formatMarkdownReport(result: StaticAnalysisResult): string {
  ...
  if (deps.manifests.length === 0) {
    ...
    for (const manifest of deps.manifests) {
      ...
      lines.push(`- **Production:** ${prod} | **Development:** ${dev}${other > 0 ? ` | **Other:** ${other}` : ''}`);
      lines.push('');
  ...
  if (!git.isGitRepo) {
    ...
    lines.push(`- **Branches:** ${bp.branchCount} (${bp.activeBranches.length} active, ${bp.staleBranches.length} stale)`);
    ...
    if (git.recentCommits.length > 0) {
      ...
      for (const c of git.recentCommits.slice(0, 10)) {
        lines.push(`| ${c.hash.slice(0, 7)} | ${truncate(c.author, 20)} | ${c.date.slice(0, 10)} | ${truncate(c.message, 50)} |`);
      }
      lines.push('');
    }
  ...
  if (Object.keys(todos.summary.byCategory).length > 0) {
    const cats = Object.entries(todos.summary.byCategory)
      .sort((a, b) => b[1] - a[1]);
  ...
  if (todos.items.length > 0) {
    ...
    for (const [category, items] of grouped) {
      ...
      for (const item of items) {
        ...
        lines.push(`| ${truncate(item.file, 40)} | ${item.line} | ${item.marker} | ${truncate(item.text, 40)} | ${refs} |`);
      }
      lines.push('');
    }
  }
  ...
  lines.push(`- **Frameworks Detected:** ${tests.frameworks.length > 0 ? tests.frameworks.join(', ') : 'None'}`);
  ...
  lines.push('---');
  lines.push('');
  lines.push(`*Generated by [handover](https://github.com/nicholasgriffintn/handover) at ${result.metadata.analyzedAt}*`);
