...
export function renderGettingStarted(ctx: RenderContext): string {
  ...
  lines.push(
    buildFrontMatter({
      ...
      document_id: '02-getting-started',
      category: 'guide',
  ...
  // Derive from language/package manager
...
/**
 * Detect primary language from static analysis file extensions.
 */
...
function detectPackageManager(ctx: RenderContext): string | null {
  ...
  // Infer from manifest file names
  const fileTree = ctx.staticAnalysis.fileTree.directoryTree;
  const fileNames = fileTree.map((e) => e.path);
  ...
  if (fileNames.some((f) => f === 'requirements.txt' || f === 'Pipfile'))
    return 'pip';
...
/**
 * Derive the install command from the package manager name.
 */
