...
- **D-01:** **Three tables, JOIN-derived per-renderer cost.** Persist:
  ...
  `handover cost`'s per-renderer view derives cost via a SQL JOIN that allocates each round's cost across the renderers that needed it. This keeps the DATA honest (totals reconcile: `Σ runs.total_cost == Σ round_runs.cost_usd`) while still presenting the user-facing per-renderer table the success criterion #1 demands. The two attribution views are defined precisely:

  - **`attributed_cost`** (default in `handover cost` output): `round_runs.cost_usd / count(renderers using that round in this run)`. Sums across renderers to exactly the run's total cost. This is the headline cost number per renderer.
  - **`standalone_cost`** (available via `handover cost --view standalone`): `SUM(round_runs.cost_usd)` over all rounds the renderer requires, with no division. Answers "what would it cost if I generated only this doc?" Will double-count when summed across renderers; UI documents that explicitly.
...
- **D-07:** **Row granularity:** 1 `runs` row per `handover generate` invocation. 1 `round_runs` row per round actually executed (so a run with `--only` selecting docs that need rounds 1,2 only writes 2 round_runs rows). 1 `renderer_runs` row per renderer that participated in the run (including `'reused'` and `'not-generated'` statuses — see D-09 for visibility rationale). `run_id` is a ULID (lexicographically sortable timestamp + random suffix). No new dependency for ULID — use a 7-line hand-rolled generator (`Date.now()` + `crypto.randomBytes(10)`) co-located in `db.ts`; the planner can swap in `ulid` from npm if they want, but a 7-line helper is fine for our threat model.
...
- **D-11:** **`--json` contract for Phase 36 consumption.** Shape (sketch — planner finalizes; format-version stability matters more than any field name):
  ...
  ```
  `formatVersion` bumps on breaking shape changes (renames, removals). Additive changes (new fields) do NOT bump. Phase 36 pins to `formatVersion: 1`. This mirrors the Phase 32 D-16 precedent exactly.
...
- **D-13:** **Rotation runs inside the same SQLite transaction as the write.** After `recordRun()` inserts the 1 + N + M rows, the SAME transaction issues two DELETEs: `DELETE FROM round_runs WHERE run_id NOT IN (recent_window)` and `DELETE FROM renderer_runs WHERE run_id NOT IN (recent_window)`, then `DELETE FROM runs WHERE run_id NOT IN (recent_window)` last (for FK-style ordering even though we won't ship explicit FK constraints — see D-15). The window is computed as the UNION of (last 100 run_ids per renderer) and (run_ids within last 90 days). Single DELETE per table, sub-millisecond on a typical DB. Predictable: every write self-cleans. Matches Phase 32 D-06 ("rebuild graph on every full run — same self-maintaining-at-write-time pattern"). Side benefit: the test surface is small — assert that after writing 101 runs, the oldest is gone.
...
- **D-17:** **Warning is emitted from the just-persisted `runs` row.** Current code at `src/ui/components.ts` checks `totalCost > costWarningThreshold` from in-memory state. Phase 33 reorders: write the `runs` row first, then read its `total_cost_usd` back and compare against `costWarningThreshold`. This satisfies TELEM-05's "sourced from the actual persisted run data" wording and gives us a single source of truth. The warning STRING reuses `renderCostWarning()` to avoid divergent UI text. If the telemetry write fails, fall back to the existing in-memory check (graceful degradation per D-21). The renderer cost-warning hook stays where it is (`src/ui/renderer.ts`); only the data source moves.
...
- **D-22:** **90/90/90/85 coverage applies** (per `.planning/codebase/TESTING.md` and Phase 32 D-23). Tests must cover:
  ...
  - Success criterion #3: rotation works — write 101 runs, assert oldest gone.
  - Success criterion #4: threshold warning fires from persisted data — set threshold to 0.001, run, assert warning string present.
  ...
  - Graceful degradation: stub better-sqlite3 to throw on insert, assert `handover generate` still exits 0.
- **D-23:** **Test fs strategy:** `memfs` will NOT work for `better-sqlite3` (native bindings need a real path). Use `os.tmpdir()` + `crypto.randomUUID()` for a fresh DB per test, cleaned in `afterEach`. Mirrors how `src/cache/round-cache.test.ts` already handles real-file tests (planner verifies pattern).
...
- **D-01 attribution math.** The proportional `attributed_cost` formula is the standard accounting choice (cost reconciles across views) but a different formula (e.g., weighted by `input_tokens` per renderer-context-window-share) is defensible. Planner reviews if research surfaces a stronger convention. The schema does not lock in the formula — it's a query.
- **D-07 ULID source.** Hand-rolled vs npm `ulid` vs `nanoid` — planner picks. Hand-rolled keeps zero-dep promise; `nanoid` is already not a dep so adding one is real cost.
