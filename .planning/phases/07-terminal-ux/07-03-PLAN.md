...
must_haves:
  truths:
    ...
    - "Retry countdown displays animated timer via onStepRetry DAG event: Round N failed · retrying in Xs..."
  ...
  key_links:
    ...
    - from: "src/cli/generate.ts"
      ...
      via: "DAG orchestrator onStepStart/onStepComplete/onStepFail/onStepRetry hooks wired to renderer.onRoundUpdate"
      pattern: "onStepRetry"
---
...
**Approach decision:** Rather than modifying all 6 `createRoundNStep` functions (which would touch 6 more files and violate the file ownership constraint), use the DAG orchestrator's `onStepStart`/`onStepComplete`/`onStepFail`/`onStepRetry` events (07-02 adds `onStepRetry` to `DAGEvents`). The `onStepComplete` hook intercepts round results with token/cost data (added by 07-02). The `onStepRetry` hook receives retry countdown information threaded from `retryWithBackoff` through `executeRound` and the provider.

**Retry countdown wiring:** Pass an `onRetry` callback to each `createRoundNStep` call. 07-02 adds the `onRetry` parameter to all 6 step creators, which thread it through `executeRound` -> `provider.complete()` -> `retryWithBackoff`. When a rate-limit retry fires, the callback maps the round number to the DAG step ID and delegates to the `onStepRetry` event on the orchestrator:
```typescript
// Create a per-round onRetry that delegates to the orchestrator's onStepRetry
const makeOnRetry = (roundNum: number) => (attempt: number, delayMs: number, reason: string) => {
  // The orchestrator's onStepRetry handler (defined above) updates display state and calls renderer
  orchestratorEvents.onStepRetry?.(`ai-round-${roundNum}`, attempt, delayMs, reason);
};

// Pass to each step creator:
createRound1Step(provider, staticAnalysis, packedContext, config, tracker, estimateTokensFn, makeOnRetry(1));
createRound2Step(provider, staticAnalysis, packedContext, tracker, estimateTokensFn, makeOnRetry(2));
// ... etc for rounds 3-6
```

This completes the full thread: `retryWithBackoff.onRetry` -> step creator closure -> `orchestratorEvents.onStepRetry` -> display state update -> `renderer.onRoundUpdate` -> animated countdown display (`Round 3 failed · retrying in 28s...`, locked user decision).

Extract the events object into a named variable (so `makeOnRetry` can reference it), then pass to `DAGOrchestrator`:
```typescript
...
This approach uses DAG event hooks for start/complete/fail/retry events, plus `onRetry` callbacks threaded through step creators (07-02 adds the `onRetry` parameter to all 6 step creators). The round tokens/cost will be available on `RoundExecutionResult` since Plan 07-02 adds those fields.
...
- Retry countdown displays animated timer when rate-limited: `Round N failed · retrying in Xs...`
