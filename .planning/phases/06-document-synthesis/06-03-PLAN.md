...
must_haves:
  ...
  key_links:
    ...
    - from: "src/renderers/registry.ts"
      ...
      via: "render function wired into DOCUMENT_REGISTRY"
      pattern: "render:"
---
...
Output: 7 renderer files + updated registry wiring. All 14 renderers complete.
</objective>
...
<task type="auto">
  <name>Task 2: Implement Edge Cases, Conventions, Deployment renderers and wire registry</name>
  <files>src/renderers/render-09-edge-cases.ts, src/renderers/render-11-conventions.ts, src/renderers/render-13-deployment.ts, src/renderers/registry.ts</files>
  <action>
    ...
    **src/renderers/registry.ts** -- Wire remaining 7 renderers:

    1. Import all 7 render functions from their files.
    2. Replace remaining placeholder `render` functions in DOCUMENT_REGISTRY for: 03-ARCHITECTURE, 05-FEATURES, 06-MODULES, 08-ENVIRONMENT, 09-EDGE-CASES, 11-CONVENTIONS, 13-DEPLOYMENT.
    3. After this task, ALL 14 entries in DOCUMENT_REGISTRY should have real render functions (no more placeholders).
    4. IMPORTANT: This plan runs in parallel with 06-02 which also modifies registry.ts. Both plans import different render functions and wire different entries. To avoid merge conflicts, structure the registry file with clear per-document sections. If plan 02 runs first, only touch the entries for docs 03, 05, 06, 08, 09, 11, 13. If plan 03 runs first, add placeholder-replacements for those 7 only, and plan 02 fills in its 7. The executor should read the current state of registry.ts before modifying.
  ...
  <done>
    Edge Cases renderer produces per-module edge case catalog from R5 data. Conventions renderer produces cross-cutting and per-module patterns from R5 data. Deployment renderer produces CI/CD and infrastructure docs from R6 data. All 14 DOCUMENT_REGISTRY entries now have real render functions.
  </done>
</task>
...
6. All 14 DOCUMENT_REGISTRY entries have non-placeholder render functions
7. Each renderer includes YAML front-matter, 2-sentence summary, cross-references
</verification>

<success_criteria>
Batch 2 of document renderers is complete: all 7 AI-heavy documents (architecture, features, modules, environment, edge-cases, conventions, deployment) have render functions. Combined with plan 02, all 14 documents in DOCUMENT_REGISTRY are wired with real renderers. The mermaid diagrams are placed in dedicated "Diagrams" sections at the end of applicable documents. Graceful degradation handles missing AI rounds.
</success_criteria>
