...
Output: Updated generate.ts with real render step, updated CLI with --audience flag.
</objective>
...
<task type="auto">
  <name>Task 1: Wire --only round skipping and --audience flag into CLI and generate</name>
  ...
  <action>
    ...
    **src/cli/generate.ts** -- Implement --only round skipping:
    ...
    6. **Update render step dependencies dynamically:**
       ...
       ```
       Import `ROUND_DEPS` from the registry or define locally (prefer import from registry -- add it to exports if not already exported).
...
<task type="auto">
  <name>Task 2: Replace render placeholder with real document generation</name>
...
2. generate.ts no longer contains placeholder render text
3. `--audience` flag registered on CLI generate command
4. `--only` flag resolves aliases and conditionally registers AI round steps
5. Render step builds RenderContext, generates documents, writes to disk
6. INDEX always generated with full status table
7. Non-selected documents listed as "not-generated" in INDEX
8. Render step dependencies dynamically computed from registered rounds
</verification>
