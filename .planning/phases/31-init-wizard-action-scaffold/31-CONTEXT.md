...
- **D-02:** Precedence rule (used in `--yes`, and as the pre-selected option in interactive mode): **cheapest detected wins**, where "cheapest" is computed from per-million-token input cost in `PROVIDER_PRESETS` (`src/providers/presets.ts`). Codex subscription beats an OpenAI API key when both exist because the subscription is flat-rate paid; do not silently consume metered API quota. Ollama (local) is treated as zero-cost — if reachable, it wins outright. The exact resulting ordering across providers is data-derived at plan time, not hard-coded here; this avoids drift when preset prices change. Rationale: SUMMARY.md pitfall #1.
...
- **D-06:** `detectMonorepo()` in `src/cli/monorepo.ts` is reused as-is (already detects npm/yarn workspaces, pnpm-workspace.yaml, Lerna, Cargo, Go). The two additions REQUIREMENTS.md INIT-02 names are added: `nx.json` → `tool: 'nx'`; `turbo.json` → `tool: 'turbo'`. Bazel and other build systems are NOT added in this phase — out of scope per INIT-02.
